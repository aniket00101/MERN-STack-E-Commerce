  ...
  return (
    <>
      ...
      <div className={`
        ...
        w-full sm:w-3/4 md:w-1/2 lg:w-[28rem] xl:w-[32rem]
        bg-[#0f172a] text-gray-200 shadow-[0_10px_40px_rgba(0,0,0,0.6)]
      ...
      `}>
        ...
        <div className='flex items-center justify-between px-5 sm:px-6 py-4 border-b border-white/10 sticky top-0 bg-[#0f172a]/95 backdrop-blur-sm z-10'>
          ...
          <button 
            onClick={toogleCartDrawer}
            className='hover:text-purple-400 transition text-lg sm:text-2xl p-1 rounded hover:bg-white/10'
            aria-label="Close Cart"
          >
            <IoMdClose className='h-6 w-6 sm:h-7 sm:w-7' />
          </button>
        </div>

        {/* Cart Items - Scrollable with bottom padding for total */}
        ...
        <div className='px-4 sm:px-6 py-6 border-t border-white/10 bg-[#0b1220]/95 backdrop-blur-sm sticky bottom-0 z-10 shrink-0'>
          ...
          <button
            ...
            disabled={false} // Add your cart empty logic here
            className='
              w-full bg-purple-600 hover:bg-purple-700 disabled:bg-gray-600 disabled:cursor-not-allowed
              py-3 sm:py-4 rounded-xl font-semibold text-sm sm:text-base transition-all
